...
class SpecialLineSegments:
    ...
    def PixelScanner(p1, p2, img):
        ...
        if p1y == p2y: 
            ...
            if large_Slope:
                ...
                if negative_Y:
                    ...
                    line_pixel_array[ : , 1 ] = np.linspace(p1y + 1 , p1y + absolute_vertical_projection + 1, number)
                line_pixel_array[ : , 0 ] = (slope * (line_pixel_array[ : , 1] - p1y)).astype(int) + p1x         
            else:
                ...
                line_pixel_array[ : , 1] = (slope * (line_pixel_array[ : , 0] - p1x)).astype(int) + p1y          
        ...
        line_pixel_array[:,2] = img[line_pixel_array[:,1].astype(int),line_pixel_array[:,0].astype(int)]
