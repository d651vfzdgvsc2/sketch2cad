...
# 移植补丁：指向本机可用的 tesseract / tessdata（可用环境变量覆盖）
import os as _os
tesseract_cmd = _os.environ.get('TESSERACT_CMD', r'D:\资料\tesseract.exe')
_tessdata = _os.environ.get('TESSDATA_PREFIX', r'C:\tessdata')
tessdata_dir_config = ' --tessdata-dir "%s"' % _tessdata
...
def run_tesseract(input_filename, output_filename_base, lang=None, boxes=False, config=None):
    ...
    if isinstance(err, bytes):  # 移植补丁：Python3 下解码 stderr
...
def image_to_string(image, lang=None, boxes=False, config=None):
    ...
    try:
        ...
        # 移植补丁：Python3 下按 utf-8 读，避免 GBK 解码失败
