"""对比不同千问 VLM 直接看图写代码的效果。"""
from __future__ import annotations

import sys
import time
from pathlib import Path

try:
    sys.stdout.reconfigure(encoding="utf-8", errors="replace")
except Exception:  # noqa: BLE001
    pass

from agents.codegen_agent import CODEGEN_DOC, _score_dxf, _wh
from tools.codegen import extract_code, run_script
from tools.vlm import ask_image

ROOT = Path(__file__).resolve().parent.parent
OUT = ROOT / "data" / "tmp" / "vlmtest"
MODELS = ["qwen-vl-max", "qwen3-vl-235b-a22b-instruct", "qwen-vl-plus"]


def run_one(model: str, img: str) -> None:
    w, h = _wh(img)
    OUT.mkdir(parents=True, exist_ok=True)
    tag = f"c_{model}"
    dxf = OUT / f"{tag}.dxf"
    prompt = (CODEGEN_DOC.format(W=w, H=h, OUT=str(dxf))
              + "\n\n请直接看这张图，写出能还原它的完整 Python 脚本。")
    t = time.time()
    try:
        raw = ask_image(img, prompt, model=model, max_tokens=4096)
    except Exception as e:  # noqa: BLE001
        print(f"{model:30} ask_image 失败: {e}")
        return
    code = extract_code(raw)
    ok, err, dxfp = run_script(code, dxf)
    if not ok:
        print(f"{model:30} 执行失败 len={len(code)}: {err[:200]}")
        return
    sc = _score_dxf(dxfp, img, OUT / f"{tag}.png")
    print(f"{model:30} SSIM={sc['ssim']:.4f} ent={sc['n_entities']:3} "
          f"code={len(code)} {round(time.time()-t,1)}s")


if __name__ == "__main__":
    import sys

    img = sys.argv[1] if len(sys.argv) > 1 else "data/real/real03.jpg"
    print("image:", img)
    for m in MODELS:
        run_one(m, img)
